...
public class BindCommand extends Command implements PluginIdentifiableCommand
{
    private final Main plugin;

    public BindCommand(Main plugin) {
        super("Bind", "XiaoBind 插件Bind指令.", "/Bind <player>");
        this.setPermission("XiaoBind.Bind");
        this.plugin = plugin;
    }
    ...
    public boolean execute(CommandSender sender, String label, String[] args)
    {
        ...
        if (args.length == 0)
        {
            ...
            {
                if((sender.hasPermission("XiaoBind.Bind")) || (sender.hasPermission("XiaoBind.Bind.Other")))
                {
        ...
        if (args.length == 1)
        {
            if(sender.hasPermission("XiaoBind.Bind.Other"))
            {
